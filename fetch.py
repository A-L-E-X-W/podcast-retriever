...
# Import modules
import rss_feed  # This file must define RSS_FEEDS as a list of RSS URLs
...
def build_legacy_data():
    """
    Build legacy podcast data using the BuildDataset class from build_dataset.py and
    update the RSS feed list using extract_raw_data.
    """
    legacy_data = []
    try:
        builder = BuildDataset(mode="alphabet")  # Uses letters a-z by default
        ...
        # Filter out rows with invalid emails (legacy data may not have them)
...
def build_full_database():
    """Combine Podchaser, RSS, legacy, and Podcast Index data to build the full podcast database."""
